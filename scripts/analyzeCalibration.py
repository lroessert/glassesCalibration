...
This directory will contain the following files:
	...
	- <condition name>_taskLog.txt: the trial timing for this run
...
### Configuration vars
startImage_path = '../task/startImage_3.jpg'
trialDur = 3000
trialWin = (500, 2500)
...
def processCalibration(condition):
    ...
    # Store the maximum number of gaze pts per trial based on the trial window and glasses sampling Hz
    idealMaxGazePts = int((trialWin[1] - trialWin[0]) / 1000 * gaze_fps[glasses])

    # Set up inputs/outputs
    dataDir, procDir, calibDir = setInputOutput(data_path, condition)

    # Find the video frame where the start image appears
    taskStartTime = findStartFrame(calibDir, dataDir)
    ...
    # Write to text files
    gazeCalibration_df, allTrials_summarized = writeTextFile(gazeCalibration_df, calibDir, allTrials_summarized)

    # Plot the results
    plotResult(gazeCalibration_df, allTrials_summarized, condition, calibDir)


def plotResult(gazeCalibration_df, allTrials_summarized, condition, calibDir):
    """
    Plot the results
    """
...
def writeTextFile(gazeCalibration_df, calibDir, allTrials_summarized):
    """
    Write to text files
    """
    ...
    gazeCalibration_colOrder = ['trial', 'ptIdx', 'col', 'row', 'trial_ts', 'task_ts', 'gaze_ts',
                                ...
                                'distance', 'angle']
    gazeCalibration_df[gazeCalibration_colOrder].to_csv(join(calibDir, 'gazeData_calibration.tsv'), sep='\t',
                                                        index=False, float_format='%.3f')
    summary_colOrder = ['trial', 'ptIdx', 'percentValid',
                        'centX', 'centY', 'centDist', 'centAngle', 'RMS']
    allTrials_summarized[summary_colOrder].to_csv(join(calibDir, 'calibrationSummary.tsv'), sep='\t', index=False,
                                                  float_format='%.3f')

    return gazeCalibration_df, allTrials_summarized


def setInputOutput(data_path, condition):
    """
    Set up input and output directories
    Create calibration folder
    Copy Task log to folder
    """

    dataDir, procDir, calibDir = setDirectories(data_path, condition)

    # Create calibration folder
    createCalibrationOutputDir(calibDir)

    # Copy Task log to folder
    copyTaskLog(data_path, condition, calibDir)

    return dataDir, procDir, calibDir


def setDirectories(data_path, condition):
    """
    Set up directories for inputs/outputs
    """
...
def createCalibrationOutputDir(calibDir):
    """
...
def copyTaskLog(data_path, condition, calibDir):
    """
...
def findStartFrame(calibDir, dataDir):
    """
    Find the video frame where the start image appears
    """
    ...
    if not os.path.exists(join(calibDir, 'startFrame.txt')):
        ...
        # Search each frame of the video
        startFrameNum, startFrame = findStartFrameFromVideo(join(dataDir, 'worldCamera.mp4'))
        # Export startFrame to calibDir
        cv2.imwrite(join(calibDir, ('startFrame_' + str(startFrameNum).zfill(4) + '.jpg')), startFrame)

        # Export startFrameNum to calibDir
        with open(join(calibDir, 'startFrame.txt'), 'w') as f:
            f.write(str(startFrameNum))
    else:
        # If startFrame already saved read the text file
        with open(join(calibDir, 'startFrame.txt'), 'r') as f:
            startFrameNum = int(f.read())

    taskStartTime = findStartImageTimestamp(dataDir, startFrameNum)

    return taskStartTime


def findStartFrameFromVideo(vidPath):
    """
...
def findStartImageTimestamp(dataDir, startFrameNum):
    """
    Find the timestamp of the start image
    """
    gazeWorld_df = pd.read_table(join(dataDir, 'gazeData_world.tsv'), sep='\t')
    frame_df = pd.read_table(join(dataDir, 'frame_timestamps.tsv'), sep='\t')
    gazeWorld_df = pd.read_table(join(dataDir, 'gazeData_world.tsv'), sep='\t')
    try:
        startImage_df = gazeWorld_df[gazeWorld_df.frame_idx == (startFrameNum - 1)].iloc[0]
        taskStartTime = startImage_df.timestamp
    except IndexError:
        findStartImageTimestampAlt(dataDir, startFrameNum)

    return taskStartTime


def findStartImageTimestampAlt(dataDir, startFrameNum):
    """
    Find the timestamp of the start image
    """
    gazeWorld_df = pd.read_table(join(dataDir, 'gazeData_world.tsv'), sep='\t')
    ...
    gazeWorld_df = pd.read_table(join(dataDir, 'gazeData_world.tsv'), sep='\t')
    try:
        # Get timestamp of start frame
        startFrame_df = frame_df[frame_df.frameNum == (startFrameNum)].iloc[0]
        taskStartTime = startFrame_df.timestamp

        # Add timestamp of first gaze timestamp to start frame timestamp
        startImage_df = gazeWorld_df[gazeWorld_df.frame_idx == (0)].iloc[0]
        taskStartTime = startImage_df.timestamp + taskStartTime
    except IndexError:
        print("No gaze data for start frame: {}".format(startFrameNum - 1))

    print("Start time: ", taskStartTime)
    return taskStartTime
...
def plotCalibrationSummary(calibSummary_df, condition, outputDir):
    ...
    for p in sorted(calibSummary_df.ptIdx):
        ...
        thisColor = cmap((p) / 25)
        xs = [thisPt.centX.iloc[0], idealLocation[0]]
        ys = [thisPt.centY.iloc[0], idealLocation[1]]
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(join(data_path, args.condition, 'processed')):
        print('Cannot find a "Processed" directory in {}'.format(join(data_path, args.condition, 'processed')))
    else:
