...
def processRecording(preprocessedDir, condition):
    ...
    border2calib_transform, mask = cv2.findHomography(borderImg_pts.reshape(-1, 1, 2), calibImg_pts.reshape(-1, 1, 2),
                                                      cv2.RANSAC, 5.0)
    ...
    while vid.isOpened():
        ...
        if (ret == True) and (frameCounter in framesToUse):
            ...
            if processedFrame['foundGoodMatch']:
                ...
                border2world_frame = projectImage2D(processedFrame['origFrame'], processedFrame['border2world'],
                                                    borderImgColor)
                ...
                for i, gazeRow in thisFrame_gazeData_world.iterrows():
                    ...
                    thisRow_df = pd.DataFrame({'gaze_ts': ts, 'worldFrame': frameNum, 'confidence': conf,
                                               ...
                                               'calibGrid_gazeX': calibGrid_gazeX, 'calibGrid_gazeY': calibGrid_gazeY},
                                              index=[i])
                    ...
                    try:
                        ...
                        cv2.circle(border_frame, (int(border_gazeX), int(border_gazeY)), dotSize, dotColor,
                                   -1)  # border frame
                        cv2.circle(calibGrid_frame, (int(calibGrid_gazeX), int(calibGrid_gazeY)), dotSize, dotColor,
                                   -1)  # calibGrid frame
        ...
        if frameCounter > np.max(framesToUse):
            ...
            try:
                colOrder = ['worldFrame', 'gaze_ts', 'confidence',
                            'world_gazeX', 'world_gazeY', 'border_gazeX', 'border_gazeY', 'calibGrid_gazeX',
                            'calibGrid_gazeY']
                gazeMapped_df[colOrder].to_csv(join(procDir, 'gazeData_mapped.tsv'), sep='\t', index=False,
                                               float_format='%.3f')
...
def processFrame(frame, frameNumber, border_kp, border_des, featureDetect, condition):
    ...
    try:
        ...
        if sufficientMatches:
            border2world_transform, mask = cv2.findHomography(border_matchPts.reshape(-1, 1, 2),
                                                              frame_matchPts.reshape(-1, 1, 2), cv2.RANSAC, 5.0)
